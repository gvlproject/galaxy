...
class Uploader:
    ...
    def request_upload(self, gsURL, gsDNS, gsUserName, gsPasword, gsToken):
        print "Uploading: " + gsURL
        if gsToken:
            r = requests.get(gsURL, cookies = { "gs-token" : gsToken }, headers = { 'Content-type': 'application/json' })
        else:
            r = requests.get(gsURL, cookies = gsLogin.get_genomespace_token(gsUserName, gsPasword, gsDNS), headers = { 'Content-type': 'application/json' })
        return r.text
